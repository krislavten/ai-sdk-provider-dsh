...
export function mapFinishReason(kind: DshTurnEndKind): LanguageModelV3FinishReason {
  switch (kind) {
    case "completed":
...
export function toV3Usage(usage: DshUsage | undefined): LanguageModelV3Usage | undefined {
...
export class DshStreamMapper {
  ...
  feedChunk(chunk: unknown): LanguageModelV3StreamPart[] {
    ...
    switch (c.type) {
      ...
      case "reasoning-delta": {
        ...
        if (typeof c.text === "string" && c.text.length > 0) {
          out.push({ type: "reasoning-delta", id: this.reasoningPartId, delta: c.text });
        }
        break;
      }
      case "tool-call-delta": {
        const id = typeof c.id === "string" ? c.id : `tool-${this.parts.length}`;
        const current = this.toolArgBuffers.get(id) ?? "";
        const delta = typeof c.argumentsDelta === "string" ? c.argumentsDelta : "";
      ...
      case "block-end": {
        ...
        if (block?.type === "tool_use" || block?.type === "tool-call") {
          const id = typeof block.id === "string" ? block.id : `tool-${this.parts.length}`;
          ...
          out.push({ type: "text-delta", id: this.textPartId, delta: block.text });
          ...
          out.push({ type: "reasoning-delta", id: this.reasoningPartId, delta: block.text });
   ...
   */
  finish(kind: DshTurnEndKind, errorMessage?: string): LanguageModelV3StreamPart[] {
    if (this.finished) return [];
    this.finished = true;
    const reason = mapFinishReason(kind);
    const terminal: LanguageModelV3StreamPart[] = [];
    if (kind === "error" && errorMessage !== undefined && errorMessage.length > 0) {

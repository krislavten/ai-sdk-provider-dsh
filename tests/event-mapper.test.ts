...
describe("DshStreamMapper", () => {
	...
	it("opens a text part but emits no delta for empty text", () => {
		const mapper = new DshStreamMapper();
		expect(
			mapper.feedChunk({ type: "text-delta", index: 0, text: "" }),
		).toEqual([{ type: "text-start", id: "text" }]);
	});
	...
	it("is inert after finish", () => {
		...
		expect(
			mapper.feedChunk({ type: "text-delta", index: 0, text: "late" }),
		).toEqual([]);
	});
});
...
describe("mapFinishReason", () => {
	it("maps dsh turn end kinds to AI SDK finish reasons", () => {
		expect(mapFinishReason("completed")).toEqual({
			unified: "stop",
			raw: "completed",
		});
		expect(mapFinishReason("max-tokens")).toEqual({
			unified: "length",
			raw: "max-tokens",
		});
		expect(mapFinishReason("aborted")).toEqual({
			unified: "error",
			raw: "aborted",
		});
		expect(mapFinishReason("interrupted")).toEqual({
			unified: "error",
			raw: "interrupted",
		});
		...
		expect(mapFinishReason("error")).toEqual({
			unified: "error",
			raw: "error",
		});
	});
